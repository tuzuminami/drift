# 00. 用語・境界定義
    ...
    ## プロダクト境界
    `DRIFT` は **Scenario and Session State Orchestrator** を提供する。
    会話UI、アカウント管理、課金、本人確認、汎用モデレーションUIは外部システムの責務とする。

    ## 主要エンティティ
    - `Scenario`: Scenarioに関する不変IDを持つドメイン要素。
- `ScenarioVersion`: ScenarioVersionに関する不変IDを持つドメイン要素。
- `Scene`: Sceneに関する不変IDを持つドメイン要素。
- `Transition`: Transitionに関する不変IDを持つドメイン要素。
- `Session`: Sessionに関する不変IDを持つドメイン要素。
- `Slot`: Slotに関する不変IDを持つドメイン要素。
- `Event`: Eventに関する不変IDを持つドメイン要素。
- `ContextPack`: ContextPackに関する不変IDを持つドメイン要素。
- `Guard`: Guardに関する不変IDを持つドメイン要素。
